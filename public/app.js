...
async function loadAvatar(img,username,updated,label=username){
  ...
  try{const response=await authFetch('/api/profile/avatar/'+encodeURIComponent(username)+'?v='+updated,{cache:'no-store'});if(!response.ok)return;const url=URL.createObjectURL(await response.blob());avatarUrls.set(key,url);img.src=url;reveal();}catch{}
}
...
$('profile-photo-picker').onchange=async event=>{const file=event.target.files?.[0];event.target.value='';if(!file)return;if(file.size>3*1024*1024)return notice('Keep the profile photo under 3 MB.');try{const response=await authFetch('/api/profile/avatar',{method:'POST',headers:{'Content-Type':file.type},body:file});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'Photo upload failed.');const url=URL.createObjectURL(file);avatarUrls.set(user+':'+result.avatarUpdated,url);profileData={...profileData,avatarUpdated:result.avatarUpdated};updateProfileVisuals();notice('Profile photo updated.');}catch(error){notice(error.message);}};
