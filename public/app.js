...
async function loadAvatar(img,username,updated,label=username){
  if(!updated||!img)return;makeAvatarViewable(img,label);const reveal=()=>{img.hidden=false;if(img.id==='profile-avatar')$('profile-avatar-placeholder').hidden=true;};const fallback=()=>{if(img.id==='profile-avatar'){$('profile-avatar-placeholder').hidden=false;img.hidden=true;}};img.onerror=fallback;const key=username+':'+updated;if(avatarUrls.has(key)){img.src=avatarUrls.get(key);reveal();return;}
  try{const response=await authFetch('/api/profile/avatar/'+encodeURIComponent(username)+'?v='+updated,{cache:'no-store'});if(!response.ok)return fallback();const blob=await response.blob();if(!blob.size)return fallback();const url=URL.createObjectURL(blob);avatarUrls.set(key,url);img.src=url;reveal();}catch{fallback();}
}
...
function renderMessage(m,{pending=false,history=false}={}){
  ...
  const bubble=document.createElement('div');bubble.className='bubble';if(!m.deleted&&(m.kind==='ai'||!currentRoom?.direct)){const name=document.createElement('strong');name.className='message-author';name.textContent=(m.kind==='ai'?'✦ ':'')+m.name;bubble.append(name);}if(m.reply&&!m.deleted){const quote=document.createElement('div');quote.className='reply-quote';const replyName=document.createElement('strong');replyName.textContent=m.reply.name;const replyText=document.createElement('span');replyText.textContent=m.reply.text;quote.append(replyName,replyText);bubble.append(quote);}if(m.text)bubble.append(document.createTextNode(m.text));
  ...
  const meta=document.createElement('span');meta.className='meta';const time=document.createElement('time');time.dateTime=new Date(m.at).toISOString();time.textContent=new Date(m.at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});if(m.edited){const edited=document.createElement('span');edited.className='edited-label';edited.textContent='edited';meta.append(edited);}meta.append(time);if(m.senderId===user&&!m.deleted){const receipt=document.createElement('span');receipt.className='receipt'+(currentRoom?.readAt>=m.at?' seen':'');receipt.textContent='✓✓';receipt.title=currentRoom?.readAt>=m.at?'Seen':'Delivered';meta.append(receipt);}bubble.append(meta);el.append(bubble);
...
function animateSendFlight(id,text){
  ...
  ghost.animate([{opacity:.15,transform:'translate(0,6px) scale(.85)'},{opacity:.8,offset:.55},{opacity:0,transform:'translate('+(to.left-from.left-from.width*.55+Math.min(to.width/2,70))+'px,'+(to.top-from.top)+'px) scale(.96)'}],{duration:240,easing:'ease-out'}).finished.finally(()=>ghost.remove());
}
...
$('attachment-button').onclick=()=>{if(!currentRoom)return;$('attachment-menu').hidden=!$('attachment-menu').hidden;$('attachment-button').setAttribute('aria-expanded',String(!$('attachment-menu').hidden));$('chat-menu').hidden=true;};
...
$('profile-photo-picker').onchange=async event=>{const file=event.target.files?.[0];event.target.value='';if(!file)return;if(file.size>3*1024*1024)return notice('Keep the profile photo under 3 MB.');const avatar=$('profile-avatar'),placeholder=$('profile-avatar-placeholder'),preview=URL.createObjectURL(file);avatar.src=preview;avatar.hidden=false;placeholder.hidden=true;try{const response=await authFetch('/api/profile/avatar',{method:'POST',headers:{'Content-Type':file.type},body:file});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'Photo upload failed.');avatarUrls.set(user+':'+result.avatarUpdated,preview);profileData={...profileData,avatarUpdated:result.avatarUpdated};updateProfileVisuals();notice('Profile photo updated.');}catch(error){URL.revokeObjectURL(preview);updateProfileVisuals();notice(error.message);}};
