...
function updateProfileVisuals(){if(!profileData)return;$('profile-name').value=profileData.displayName||user;$('profile-about').value=profileData.about||'';$('profile-read-receipts').checked=profileData.readReceipts!==false;$('profile-avatar-placeholder').textContent=(profileData.displayName||user).charAt(0).toUpperCase();if(profileData.avatarUpdated)loadAvatar($('profile-avatar'),user,profileData.avatarUpdated,profileData.displayName||user);}
