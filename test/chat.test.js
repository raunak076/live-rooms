...
test('real-time delivery, DM isolation, ownership, deduplication, AI routing and persistence',async()=>{
  const folder=mkdtempSync(join(tmpdir(),'live-rooms-'));const dbPath=join(folder,'test.db');let aiCalls=0;
  let chat=createChat({dbPath,generate:async history=>{aiCalls++;assert.match(history.at(-1).text,/@gemini/);return 'Mocked AI answer';}});
  ...
  try{
    ...
    await rpc(bob,'auth',{username:'bob',password:'strong-password',register:true});await rpc(eve,'auth',{username:'eve',password:'strong-password',register:true});
