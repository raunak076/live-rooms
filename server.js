...
export function createChat({generate=askGemini,dbPath='data/chat.db',pushNotification,voiceClone,voiceReviewKey=process.env.VOICE_REVIEW_KEY}={}) {
  ...
  async function geminiTranscribe(audio,mime){if(!process.env.GEMINI_API_KEY)throw Object.assign(new Error('Voice transcription is not configured.'),{statusCode:503});const response=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+encodeURIComponent(process.env.GEMINI_TRANSCRIBE_MODEL||'gemini-2.5-flash')+':generateContent',{method:'POST',signal:AbortSignal.timeout(120000),headers:{'Content-Type':'application/json','x-goog-api-key':process.env.GEMINI_API_KEY},body:JSON.stringify({contents:[{role:'user',parts:[{text:'Transcribe this audio exactly. Return only the spoken words, with no explanation.'},{inline_data:{mime_type:mime,data:audio.toString('base64')}}]}],generationConfig:{temperature:0}})}),data=await response.json().catch(()=>({})),text=data.candidates?.[0]?.content?.parts?.map(part=>part.text||'').join('').trim();if(!response.ok||!text)throw Object.assign(new Error(data.error?.message||'The recording could not be transcribed.'),{statusCode:502});return text;}
  async function fishTranscribe(audio,mime){const form=new FormData();form.append('audio',new Blob([audio],{type:mime}),'voice'+(voiceMimeExtensions.get(mime)||'.webm'));form.append('ignore_timestamps','true');const response=await fetch('https://api.fish.audio/v1/asr',{method:'POST',signal:AbortSignal.timeout(120000),headers:{Authorization:'Bearer '+process.env.FISH_AUDIO_API_KEY,model:'transcribe-1'},body:form}),data=await response.json().catch(()=>({}));if(!response.ok||!data.text?.trim())throw Object.assign(new Error(data.message||data.reason||'Fish Audio could not understand this recording.'),{statusCode:response.status===402?503:502});return data.text.trim();}
  async function transcribeVoice(audio,mime){let geminiError;if(process.env.GEMINI_API_KEY)try{return await geminiTranscribe(audio,mime);}catch(error){geminiError=error;}if(process.env.FISH_AUDIO_API_KEY)try{return await fishTranscribe(audio,mime);}catch(error){if(error.statusCode===503)throw Object.assign(new Error('Voice transcription quota is unavailable. Add Fish Audio balance or check the Gemini key.'),{statusCode:503});throw error;}throw geminiError||Object.assign(new Error('Configure Gemini transcription before using cloned voices.'),{statusCode:503});}
  async function fishSynthesize(text,model,sample){const sampleText=model.sample_text||await transcribeVoice(sample,model.sample_mime);if(!model.sample_text)run('UPDATE voice_models SET sample_text=? WHERE id=?',sampleText,model.id);const response=await fetch('https://api.fish.audio/v1/tts',{method:'POST',signal:AbortSignal.timeout(120000),headers:{Authorization:'Bearer '+process.env.FISH_AUDIO_API_KEY,'Content-Type':'application/msgpack',model:'s2.1-pro-free'},body:messagePack({text,references:[{audio:sample,text:sampleText}],format:'mp3',mp3_bitrate:128,normalize:true,latency:'balanced'})});if(!response.ok){const data=await response.json().catch(()=>({}));throw Object.assign(new Error(data.message||data.reason||'Fish Audio voice generation failed.'),{statusCode:response.status===402?503:502});}return{buffer:watermarkMp3(Buffer.from(await response.arrayBuffer()),model),mime:'audio/mpeg',watermarked:true};}
  async function fishCloneVoice(source,mime,model,sample){return fishSynthesize(await transcribeVoice(source,mime),model,sample);}
  ...
  app.post('/api/voices/:id/preview',express.json({limit:'2kb'}),async(req,res)=>{
    const username=sessionUser(req),model=get('SELECT * FROM voice_models WHERE id=?',req.params.id);if(!username)return res.status(401).json({error:'Sign in again before testing this voice.'});if(!model||model.owner!==username||model.status!=='approved'||model.revoked_at)return res.status(404).json({error:'This approved voice is unavailable.'});const text=typeof req.body?.text==='string'?req.body.text.trim().slice(0,160):'';if(!text)return res.status(400).json({error:'Enter preview text.'});
    try{let result;if(process.env.FISH_AUDIO_API_KEY&&model.provider_voice_id==='fish:reference')result=await fishSynthesize(text,model,await readFile(join(voiceRoot,model.sample_file)));else if(process.env.ELEVENLABS_API_KEY&&model.provider_voice_id){const response=await fetch('https://api.elevenlabs.io/v1/text-to-speech/'+encodeURIComponent(model.provider_voice_id)+'?output_format=mp3_44100_128',{method:'POST',signal:AbortSignal.timeout(120000),headers:{'Content-Type':'application/json','xi-api-key':process.env.ELEVENLABS_API_KEY},body:JSON.stringify({text,model_id:'eleven_multilingual_v2'})});if(!response.ok){const data=await response.json().catch(()=>({}));throw Object.assign(new Error(data.detail?.message||data.detail||'Voice preview failed.'),{statusCode:502});}result={buffer:watermarkMp3(Buffer.from(await response.arrayBuffer()),model),mime:'audio/mpeg',watermarked:true};}else throw Object.assign(new Error('Voice preview is not available for this provider.'),{statusCode:503});res.setHeader('Cache-Control','no-store');res.type(result.mime).send(result.buffer);}catch(error){console.error('Voice preview failed',model.id,error.message);res.status(error.statusCode||500).json({error:error.message||'Voice preview failed.'});}
  });
