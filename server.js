...
export function createChat({generate=askGemini,dbPath='data/chat.db',pushNotification}={}) {
  ...
  io.on('connection',socket=>{
    ...
    handler('enter',(p,ack)=>{
      ...
      run('DELETE FROM conversation_hides WHERE room_id=? AND username=?',room.id,socket.data.user);joinUser(socket.data.user,room.id);ack({room:snapshot(room.id,socket.data.user)});
    });
